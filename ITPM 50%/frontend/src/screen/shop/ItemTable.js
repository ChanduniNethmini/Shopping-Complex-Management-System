...
import { Typography, Grid, Button, TextField, Table, TableContainer, TableHead, TableBody, TableRow, TableCell, IconButton, Card } from "@material-ui/core";
import { Close, Delete as DeleteIcon, Edit as EditIcon } from "@material-ui/icons";
...
const ItemHome = () => {
    ...
    const validateField = (fieldName, value) => {
        ...
        switch (fieldName) {
            ...
            case "itemDescription":
                errorMessage = value.trim() === "" ? "Item Description is required" : "";
                break;
            case "itemPrice":
    ...
    const handleSubmit = (e) => {
        ...
        if (Object.values(errors).every((x) => x === "")) {
            ...
            axios.post(API_URL + "/item/add", formDataCopy)
                .then((res) => {
    ...
    const handleDelete = (id) => {
        axios.delete(API_URL + "/item/delete/" + id)
            .then((res) => {
    ...
    const handleUpdate = (e) => {
        e.preventDefault();
        let formDataCopy = { ...formData, itemImage: url };
        axios.put(API_URL + "/item/update/" + id, formDataCopy).then((res) => {
            setOpen(false);
            clearForm();
            setId("");
            setUrl('');
                                    ...
                                    {dataList.map((item) => (
                                        <TableRow key={item._id}>
                                            ...
                                            <TableCell align="right">
                                                <Image cloudName={CLOUD_NAME} publicId={item.itemImage} width="50" height="50" />
                                            </TableCell>
                                            <TableCell align="right">
